...
export class SignUpComponent implements OnInit {
  ...
  createProfile(){
    ...
    const headers = new HttpHeaders({
      ...
      'Access-Control-Allow-Origin': 'http://localhost:4200, http://localhost:3001',
    ...
    this.http.post<{[account_creation: string]: any}>('http://localhost:3001/users/signup', {firstName: firstname, lastName: lastname, username: username, email: email, phonenumber: phonenumber, password: password, zip: zip, responseType: 'application/json'}, {headers: headers}).subscribe((res) => {
    console.log(res);  
